...
protected:
	...
	// 4 pitched oscillators used in FM algorithms
	COscillator* m_pOsc1;
	COscillator* m_pOsc2;
	COscillator* m_pOsc3;
	COscillator* m_pOsc4;
	...
	// noise generator
	COscillator* m_pNoiseGen;

	// 2 filters (in this project only one filter is used)
	CFilter* m_pFilter1;
	CFilter* m_pFilter2;

	// 4 pitched oscillator EGs
	...
	// noise generator EG
	CEnvelopeGenerator m_NoiseGenEG;
...
public:
	inline virtual void initGlobalParameters(globalSynthParams* pGlobalParams)
	{
		...
		if(m_pNoiseGen)m_pNoiseGen->initGlobalParameters(&m_pGlobalSynthParams->noiseGenParams);
		...
		m_NoiseGenEG.initGlobalParameters(&m_pGlobalSynthParams->noiseGenEGParams);
		m_FilterEG.initGlobalParameters(&m_pGlobalSynthParams->filterEGParams);
	...
	inline void noteOn(UINT uMIDINote, UINT uMIDIVelocity, double dFrequency, double dLastNoteFrequency)
	{
		...
		if(!m_bNoteOn && !m_bNotePending)
		{
			...
			{	
				...
				if(m_pNoiseGen)m_pNoiseGen->m_dOscFo = m_dPortamentoStart;
			}
			else
			{
				...
				if(m_pNoiseGen)m_pNoiseGen->m_dOscFo = m_dOscPitch;
			}
			...
			if(m_pNoiseGen)m_pNoiseGen->m_uMIDINoteNumber = m_uMIDINoteNumber;
			...
			if(m_pNoiseGen)m_pNoiseGen->startOscillator();
	...
	inline virtual bool doVoice(double& dLeftOutput, double& dRightOutput)
	{
		...
		if(isVoiceDone() || m_bNotePending)
		{
			...
			if(isVoiceDone() && !m_bNotePending)
			{
				...
				if(m_pNoiseGen)m_pNoiseGen->stopOscillator();
			...
			else if(m_bNotePending && (isVoiceDone() || inLegatoMode())) // note pending so turn it on
			{
				...
				if(m_pNoiseGen)m_pNoiseGen->m_dOscFo = dPitch;
				...
				if(m_pNoiseGen)m_pNoiseGen->update();
		...
		if(m_dPortamentoInc > 0.0 && m_pOsc1->m_dOscFo != m_dOscPitch)
		{
			...
			else
			{
				...
				if(m_pNoiseGen) m_pNoiseGen->m_dOscFo = dPortamentoPitch;
