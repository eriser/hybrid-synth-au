...
void CVoice::initializeModMatrix(CModulationMatrix* pMatrix)
{
	...
	/*pRow = createModMatrixRow(SOURCE_MODWHEEL,
...
void CVoice::setSampleRate(double dSampleRate)
{
	...
	if(m_pNoiseGen)m_pNoiseGen->setSampleRate(dSampleRate);
	...
	m_NoiseGenEG.setSampleRate(dSampleRate);
	m_FilterEG.setSampleRate(dSampleRate);
...
void CVoice::prepareForPlay()
{
	...
	m_NoiseGenEG.m_pModulationMatrix = &m_ModulationMatrix;
	m_NoiseGenEG.m_uModDestEGOutput = SOURCE_NOISE_GEN_EG;
	m_NoiseGenEG.m_uModDestBiasedEGOutput = SOURCE_BIASED_NOISE_GEN_EG;
	m_NoiseGenEG.m_uModSourceEGAttackScaling = DEST_NOISE_GEN_EG_ATTACK_SCALING;
	m_NoiseGenEG.m_uModSourceEGDecayScaling = DEST_NOISE_GEN_EG_DECAY_SCALING;
	m_NoiseGenEG.m_uModSourceSustainOverride = DEST_NOISE_GEN_EG_SUSTAIN_OVERRIDE;

	m_FilterEG.m_pModulationMatrix = &m_ModulationMatrix;
	m_FilterEG.m_uModDestEGOutput = SOURCE_FILTER_EG;
	m_FilterEG.m_uModDestBiasedEGOutput = SOURCE_BIASED_FILTER_EG;
	m_FilterEG.m_uModSourceEGAttackScaling = DEST_FILTER_EG_ATTACK_SCALING;
	m_FilterEG.m_uModSourceEGDecayScaling = DEST_FILTER_EG_DECAY_SCALING;
	m_FilterEG.m_uModSourceSustainOverride = DEST_FILTER_EG_SUSTAIN_OVERRIDE;
	...
	if(m_pNoiseGen)
	{
		m_pNoiseGen->m_pModulationMatrix = &m_ModulationMatrix;
		m_pNoiseGen->m_uModSourceFo = DEST_NOISE_GEN_FO;
		m_pNoiseGen->m_uModSourcePulseWidth = DEST_NOISE_GEN_PULSEWIDTH;
		m_pNoiseGen->m_uModSourceAmp = DEST_NOISE_GEN_OUTPUT_AMP; 
	}
	...
	if(m_pFilter2)
	{
		...
		m_pFilter2->m_uModSourceFc = DEST_FILTER2_FC;
...
void CVoice::reset()
{
	...
	if(m_pNoiseGen)m_pNoiseGen->reset();
