...
public:
	...
	// sources that we read from indexes in m_pModulationMatrix->Sources[]
	UINT m_uModSourceFo; 
	UINT m_uModSourcePulseWidth; 
	UINT m_uModSourceAmp;

	// destinations that we write to indexes in m_pModulationMatrix->Destinations[]
	...
	// for LFOs
	enum {sine,tri,usaw,dsaw,square,rsh,qrsh};
...
public:
	...
	// Check and wrap the modulo. Returns true if modulo wrapped
	...
	// for FM only (not used in Yamaha DX or HybridSynth)
	inline void setFoModLin(double dMod){m_dFoModLin = dMod;}

	// for Yamaha DX or HybridSynth
	...
	{		
		...
		// bound Fo (can go outside for FM/PM mod) +/- 20480 for FM/PM
		if(m_dFo > OSC_FO_MAX)
			m_dFo = OSC_FO_MAX;
		if(m_dFo < -OSC_FO_MAX)
			m_dFo = -OSC_FO_MAX;
