...
void CEnvelopeGenerator::reset()
{
	...
	// Note: sustain pedal may still be down so it is a sticky variable
	m_bReleasePending = false;
	...
	// if reset to zero, clear else let it stay frozen
	if(m_bResetToZero)
	{
		m_dEnvelopeOutput = 0.0;
	} 
...
// changes to the decay rate OR the sustain level require an update in analog mode
